...
Reconciled through Mission 019: 2026-08-30 CT
...
The output guardrails are now enforced for real output (Mission 019): every
worker seat and the Moderator carry a provisional 40k-char hard cap (exact
deterministic truncation via `guardrails.py`, terminal `finish_reason:
"modelmix_output_cap"`, honestly distinct from normal completion, user
cancellation, provider/model termination, failure, and timeout) plus a one-shot
20k-char output warning (`seat_output_warning`/`moderator_output_warning`),
backed by module constants that a later Settings mission should make
configurable. The provider/account usage warning stays explicitly open because
no authoritative quota/rate-limit data exists anywhere to compare against.
Mission 015's telemetry truth layer is rendered as compact per-seat footers
(Mission 018) with no telemetry dashboard, usage kept opaque and
